...
export const forgotPassword = async (req, res) => {
  try {
    ...
    if (user.lastOTPRequestTime) {
      const timeSinceLastRequest = (Date.now() - user.lastOTPRequestTime.getTime()) / 1000;
      if (timeSinceLastRequest < 60) {
        const remainingTime = Math.ceil(60 - timeSinceLastRequest);
        return res.status(429).json({
          success: false,
    ...
    // Rate limiting - Check 24-hour limit (3 attempts)
    if (user.otpRequestResetTime) {
      const timeSinceReset = Date.now() - user.otpRequestResetTime.getTime();
      if (timeSinceReset < 24 * 60 * 60 * 1000) {
        // Within 24 hours
        if (user.otpRequestCount >= 3) {
          return res.status(429).json({
            success: false,
            message: "You've reached the maximum OTP requests (3) allowed in 24 hours",
            limitExceeded: true,
          });
        }
      } else {
        // Reset counter after 24 hours
        user.otpRequestCount = 0;
        user.otpRequestResetTime = Date.now();
      }
    } else {
      user.otpRequestResetTime = Date.now();
    }
...
export const verifyResetOTP = async (req, res) => {
  try {
    ...
    // Generate a temporary reset token for the password reset page
    const resetToken = crypto.randomBytes(20).toString('hex');
    user.resetPasswordToken = crypto
      .createHash('sha256')
      .update(resetToken)
    ...
    res.json({
      success: true,
      message: 'OTP verified successfully',
      resetToken, // Send token to frontend for password reset
    });
  } catch (error) {
